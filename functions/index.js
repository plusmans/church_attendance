...
// ✅ 1. 일반 중보기도 등록 시 알림 (prayers_requests 컬렉션 감지)
// 한글 경로 오류를 피하기 위해 {departmentId} 와일드카드 사용
exports.sendPrayerNotification = onDocumentCreated("departments/{departmentId}/prayers_requests/{prayerId}", async (event) => {
  if (event.params.departmentId !== '중등부') return; // 중등부가 아니면 함수 종료 (안전장치)
  ...
  // 실제 저장되는 본문 필드명이 'content'가 아니라면 그에 맞게 수정해 주세요.
  const content = prayerData.content || '새로운 기도제목이 등록되었습니다.'; 
...
exports.sendUrgentPrayerNotification = onDocumentCreated("departments/{departmentId}/urgent_prayers/{prayerId}", async (event) => {
  if (event.params.departmentId !== '중등부') return; // 중등부가 아니면 함수 종료
  ...
  // 실제 저장되는 본문 필드명이 'content'가 아니라면 그에 맞게 수정해 주세요.
  const content = prayerData.content || '새로운 긴급 기도제목이 등록되었습니다.'; 
